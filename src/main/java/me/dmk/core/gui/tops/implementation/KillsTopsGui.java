...
public class KillsTopsGui {
    ...
    public void open(Player player) {
        ...
        Gui gui = Gui.gui()
                .title(ComponentUtil.text(circle + " <light_purple>Topki zabójstw " + circle))
                .rows(5)
        ...
        for (int i = 0; i < profileList.size(); i++) {
            ...
            GuiItem item = SkullStorage.createPlayerHead(profile.getUuid())
                    ...
                    .lore(ComponentUtil.asList(
                            "",
                            circle + " <gray>Gracz posiada <light_purple>" + statistics.getKills() + " <gray>zabójstw<dark_gray>.",
                            ""
                    ))
                    .asGuiItem();
