...
@AllArgsConstructor
public class KillsTopsGui extends ItemStorage {
    ...
    public void open(Player player) {
        Gui gui = Gui.gui()
                .title(ComponentUtil.text(this.circle + " <light_purple>Topki zabójstw " + this.circle))
                .rows(5)
        ...
        Bson sort = Indexes.descending("profileStatistics.kills");
        ...
        for (int i = 0; i < profileList.size(); i++) {
            ...
            GuiItem item = SkullStorage.createPlayerHead(profile.getUuid())
                    ...
                    .lore(ComponentUtil.asList(
                            "",
                            "<red>" + SymbolUtil.getSword() + " <gray>Gracz posiada <red>" + statistics.getKills() + " <gray>zabójstw<dark_gray>.",
                            ""
                    ))
                    .asGuiItem();
