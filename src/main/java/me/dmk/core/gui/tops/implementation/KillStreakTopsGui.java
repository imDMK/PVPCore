...
@AllArgsConstructor
public class KillStreakTopsGui {
    ...
    public void open(Player player) {
        ...
        Gui gui = Gui.gui()
                .title(ComponentUtil.text(circle + " <light_purple>Topka serii zabójstw " + circle))
                .rows(5)
        ...
        for (int i = 0; i < profileList.size(); i++) {
            ...
            GuiItem item = SkullStorage.createPlayerHead(profile.getUuid())
                    ...
                    .lore(ComponentUtil.asList(
                            "",
                            circle + " <gray>Gracz posiada <light_purple>" + statistics.getHighestKillStreak() + " <gray>zabójstw pod rząd<dark_gray>.",
                            ""
                    ))
                    .asGuiItem();
