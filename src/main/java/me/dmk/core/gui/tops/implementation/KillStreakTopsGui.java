...
@AllArgsConstructor
public class KillStreakTopsGui extends ItemStorage {
    ...
    public void open(Player player) {
        Gui gui = Gui.gui()
                .title(ComponentUtil.text(this.circle + " <light_purple>Topka serii zabójstw " + this.circle))
                .rows(5)
        ...
        Bson sort = Indexes.descending("profileStatistics.highestKillStreak");
        ...
        for (int i = 0; i < profileList.size(); i++) {
            ...
            GuiItem item = SkullStorage.createPlayerHead(profile.getUuid())
                    ...
                    .lore(ComponentUtil.asList(
                            "",
                            SymbolUtil.getSword("<red>") + " <gray>Gracz posiada <red>" + statistics.getHighestKillStreak() + " <gray>" + killSteakFormat + " pod rząd<dark_gray>.",
                            ""
                    ))
                    .asGuiItem();
